...
- CMake 3.16+
- Qt 6.6+
- `libmpv` (开发库，项目会优先规整到本地 `.deps/mpv`)

除 `Qt` 外，其余核心 C/C++ 编译依赖会尽量统一收敛到项目本地依赖目录：
- `ffmpeg` / `libtorrent`：由 `vcpkg.json` + `.deps/vcpkg` 管理
- `libmpv`：统一优先发现 `.deps/mpv`，不同平台的来源会被规整到这一目录后再参与编译和打包
...
# 3. 运行构建脚本（会自动初始化项目内 .deps/vcpkg，并规整 libmpv 到 .deps/mpv）
